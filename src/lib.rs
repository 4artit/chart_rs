...
//! | [`StateNode`] | yes | A state plus variables scoped to it |
//! | [`Edge`] | none (static) | One row of the table |
//! | [`Machine`] | current tag + state nodes | The executor |
//!
//! # Declaration macros
//!
//! | Macro | Generates |
...
//! | [`state!`] | A [`StateNode`] impl without variables |
...
pub trait Domain: Sized + 'static {
    /// State identifier. Payloads live in [`StateNode`]; only the tag is here.
    ///
    ...
    /// cannot be baked into [`Edge::run`] — that field is `&'static`, so it holds
    /// compile-time constants only. This is also the sole way an internal
    /// transition can act on a payload, since it runs neither `on_enter` nor
    /// `on_exit`.
    ///
